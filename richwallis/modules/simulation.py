...
def simulate_hidden(
    ...
    data = {
        ...
        'action_seqs': [],
        'item_seqs': [],
        'posterior_means_seqs': [],
        'posterior_precisions_seqs': [],
    }
    if include_hidden:
        data['hidden_seqs'] = []
    if include_logits:
    ...
    for _ in range(num_trials):
        ...
        action_seq_ep = []
        item_seq_ep = []
        posterior_means_seq_ep = []
        posterior_precisions_seq_ep = []
        if include_hidden:
            hidden_seq_ep = []
        if include_logits:
            logits_seq_ep = []
        ...
        with torch.no_grad():
            ...
            while not done:
                ...
                # record results for the timestep
                action_seq_ep.append(int(action))
                item_seq_ep.append(info['item'])
                posterior_means_seq_ep.append(info['posterior_means'])
                posterior_precisions_seq_ep.append(info['posterior_precisions'])
                if include_hidden:
                    hidden_seq_ep.append(process_hidden(states, net_type))
                if include_logits:
                    logits_seq_ep.append(net.policy_net.logits.squeeze().tolist())
            ...
            # step the network one more time step without stepping the env
            action, policy, log_prob, entropy, value, states = net(
                obs, states, action_mask
            )
            if greedy:
            ...
            data['values'].append(list(env.env.values))
            data['action_seqs'].append(action_seq_ep)
            data['item_seqs'].append(item_seq_ep)
            data['posterior_means_seqs'].append(posterior_means_seq_ep)
            data['posterior_precisions_seqs'].append(posterior_precisions_seq_ep)
            if include_hidden:
                data['hidden_seqs'].append(hidden_seq_ep)
            if include_logits:
                data['logits_seqs'].append(logits_seq_ep)
...
def preprocess(data, args, merge_fixations = False):
    ...
    for i in range(num_trials):
        ...
        decision_ep = action_seq_ep[-1] - args.num_bandits
