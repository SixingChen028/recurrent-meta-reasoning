...
"""
Helper function to compute Q values
"""

LR = 0.3 # 0.25

def get_pr_q_values_seq(fixation_seq, child_dict, parent_dict, points, root_node):
    """
    Predecessor-style MC backup: when node is revealed, update Q(node) and
    all ancestors toward points[node] with a learning rate.
    """
    q_values = np.zeros(11)
    node_mask = np.zeros(11)
    node_mask[root_node] = 1.

    q_values_seq = []

    for t, node in enumerate(fixation_seq):
        q_values_seq.append(q_values.copy())  # snapshot before update

        if node_mask[node] == 0:
            cur = node
            while cur is not None:
                q_values[cur] = q_values[cur] + LR * max(0, points[node])
                cur = parent_dict.get(cur, None)

        node_mask[node] = 1.

    q_values_seq.append(q_values.copy())  # snapshot after last update
    return np.array(q_values_seq)  # shape: (T+1, 11)
...
for i in range(num_trials):
    ...
    # compute clipped q values sequence
    pr_q_values_seq_ep = get_pr_q_values_seq(
        fixation_seq_ep, child_dict_ep, parent_dict_ep, points_ep, root_node_ep
    )
    ...
    for t in range(1, len(fixation_seq_ep) - 1):
        ...
        delta_q_parent = pr_q_values_seq_ep[t + 1, par] - pr_q_values_seq_ep[t, par]
...
for i in range(num_trials):
    ...
    for t in range(1, len(fixation_seq_ep) - 1):
        ...
        delta_q_grandpar = pr_q_values_seq_ep[t + 1, grandpar] - pr_q_values_seq_ep[t, grandpar]
...
for i in range(num_trials):
    ...
    for t in range(1, len(fixation_seq_ep) - 1):
        ...
        delta_q_grandgrandpar = pr_q_values_seq_ep[t + 1, grandgrandpar] - pr_q_values_seq_ep[t, grandgrandpar]
...
with open(os.path.join(exp_path, 'logger_backup_predecessor_opt.pkl'), 'wb') as f:
