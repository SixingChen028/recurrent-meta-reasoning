...
def simulate(
    ...
    for _ in range(num_trials):
        ...
        with torch.no_grad():
            ...
            data['child_dicts'].append(env.env.graph.child_dict)
            data['parent_dicts'].append(env.env.graph.parent_dict)
            data['root_nodes'].append(int(env.env.graph.root_node))
            data['leaf_nodes'].append(list(env.env.graph.leaf_nodes))
            data['points'].append(list(env.env.graph.points))
            data['cum_points'].append(list(env.env.graph.cum_points))
            data['depths'].append(list(env.env.graph.get_depths()))
...
def preprocess(data, args, merge_fixations = False):
    ...
    for i in range(num_trials):
        ...
        decision_seq_ep = [action - args.num_nodes for action in action_seq_ep if (action >= args.num_nodes and action < 2 * args.num_nodes)]
