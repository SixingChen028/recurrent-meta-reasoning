...
def get_q_values_clipped(child_dict, points, num_node = 11, gamma = 1):
    """
    ...
    def _rdfs(node):
        ...
        # set the q value
        q_values[node] = points[node] + gamma * max(0, max_child_q_value)
...
for i in range(num_trials):
    ...
    # compute clipped q values sequence
    clipped_q_values_seq_ep = []
    for node in fixation_seq_ep:
        clipped_q_values_t = get_q_values_clipped(child_dict_ep, points_ep * node_mask_ep)
        clipped_q_values_seq_ep.append(clipped_q_values_t)
        node_mask_ep[node] = 1
    clipped_q_values_seq_ep = np.array(clipped_q_values_seq_ep)
    ...
    for t in range(1, len(fixation_seq_ep) - 1):
        ...
        delta_q_parent = clipped_q_values_seq_ep[t + 1, par] - clipped_q_values_seq_ep[t, par]
...
for i in range(num_trials):
    ...
    for t in range(1, len(fixation_seq_ep) - 1):
        ...
        delta_q_grandpar = clipped_q_values_seq_ep[t + 1, grandpar] - clipped_q_values_seq_ep[t, grandpar]
...
for i in range(num_trials):
    ...
    for t in range(1, len(fixation_seq_ep) - 1):
        ...
        delta_q_grandgrandpar = clipped_q_values_seq_ep[t + 1, grandgrandpar] - clipped_q_values_seq_ep[t, grandgrandpar]
...
with open(os.path.join(exp_path, 'logger_backup_bellman_opt.pkl'), 'wb') as f:
