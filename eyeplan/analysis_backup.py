...
for i in range(num_trials):
    ...
    for t in range(1, len(fixation_seq_ep) - 1):
        ...
        # delta logit of the PARENT: logit[t+1, parent] - logit[t, parent]
        parent_action_idx = args.num_nodes + par  # adjust if your indexing differs
        delta_logit_parent = (logits_seq_ep[t + 1, parent_action_idx] - logits_seq_ep[t, parent_action_idx])

        records.append({
            'revealed_reward': revealed_reward,
            'sibling_value':   sibling_value,
            'delta_logit_parent': delta_logit_parent,
        })
...
for i in range(num_trials):
    ...
    for t in range(1, len(fixation_seq_ep) - 1):
        ...
        # delta logit of the GRANDPARENT
        grandpar_action_idx = args.num_nodes + grandpar
        delta_logit_grandpar = logits_seq_ep[t + 2, grandpar_action_idx] - logits_seq_ep[t + 1, grandpar_action_idx]

        records.append({
            'revealed_reward': points_ep[node],
            'delta_q_par':    delta_q_par,
            'par_sibling_q':      par_sibling_q,
            'delta_logit_grandpar': delta_logit_grandpar,
        })
...
for i in range(num_trials):
    ...
    for t in range(1, len(fixation_seq_ep) - 1):
        ...
        # delta logit of the GRANDGRANDPARENT
        grandgrandpar_action_idx = args.num_nodes + grandgrandpar
        delta_logit_grandgrandpar = logits_seq_ep[t + 2, grandgrandpar_action_idx] - logits_seq_ep[t + 1, grandgrandpar_action_idx]

        records.append({
            'revealed_reward': points_ep[node],
            'delta_q_grandpar':          delta_q_grandpar,
            'grandpar_sibling_q':        grandpar_sibling_q,
            'delta_logit_grandgrandpar': delta_logit_grandgrandpar,
        })
...
with open(os.path.join(exp_path, 'logger_backup.pkl'), 'wb') as f:
